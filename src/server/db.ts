...
export const db =
  ...
  new PrismaClient({
    datasourceUrl: `postgresql://${env.DB_USER}:${env.DB_PASSWORD}@${env.DB_HOST}/${env.DB_NAME}?${env.DB_SOCKET ? `host=${env.DB_SOCKET}&` : ''}sslmode=require&sslcert=${env.SSL_CERT_PATH}&sslidentity=${env.SSL_IDENTITY_PATH}&sslpassword=${env.SSL_CLIENT_IDENTITY_PASS}&sslaccept=accept_invalid_certs`,
    log:
